...
namespace KWUtils
{
    ...
    public static class KWChunk
    {
        ...
        */
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static List<T[]> GetGridOrderedByListedChunk<T>(T[] unorderedIndices, in GridData gridData)
        where T : struct
        {
            using NativeArray<T> nativeUnOrderedIndices = unorderedIndices.ToNativeArray();
            using NativeArray<T> nativeOrderedIndices = new (unorderedIndices.Length, Allocator.TempJob, NativeArrayOptions.UninitializedMemory);
            ...
            List<T[]> chunkCells = new List<T[]>(gridData.TotalChunk);
            jobHandle.Complete();
            for (int i = 0; i < gridData.TotalChunk; i++)
            {
                T[] temp = nativeOrderedIndices.Slice(i * gridData.TotalCellInChunk, gridData.TotalCellInChunk).ToArray();
                chunkCells.Add(temp);
            }
            return chunkCells;
        }
