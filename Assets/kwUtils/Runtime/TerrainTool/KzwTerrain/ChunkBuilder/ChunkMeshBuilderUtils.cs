...
namespace KWUtils
{
    ...
    {
        public static JobHandle SetNoiseJob(NoiseData noiseData, ChunkData chunk, int2 chunkCoord, NativeArray<float> noiseMap, JobHandle jobHandle = default)
        {
            NativeArray<float2> octaveOffsets = new(noiseData.Octaves, TempJob, UninitializedMemory);
        ...
        public static JobHandle SetMeshJob(ChunkData chunkSetting, MeshData meshData, NativeArray<float> noiseMap, JobHandle jobHandle = default)
        {
            JMeshDatas job = new()
            {
        ...
        public static JobHandle SetNormalsJob(ChunkData chunkSetting, MeshData meshData, JobHandle jobHandle = default)
        {
            JNormals job = new()
            {
                MapSizeX = chunkSetting.NumVerticesPerLine,
        ...
        public static JobHandle SetTangentsJob(ChunkData chunkSettings, MeshData meshData, JobHandle jobHandle = default)
        {
            NativeArray<float3> tangent1 = new(chunkSettings.VerticesCount,TempJob,UninitializedMemory);
            NativeArray<float3> tangent2 = new(chunkSettings.VerticesCount,TempJob,UninitializedMemory);
...
// =====================================================================================================================
        ...
        //TANGENT(Vertex) : MUST BE DONE AFTER NORMALS!
        //==================================================================================================================
        [BurstCompile]
        ...
        public struct JNormals : IJobFor
        {
            ...
            public void Execute(int index)
            {
                (int x, int y) = KWGrid.GetXY(index, MapSizeX);
                float3 normal = GetNormal(x, y, MapSizeX);
                Normals[index] = normal;
            }

            private float3 GetNormal(int x, int y, int width) //quad split from bottom-right to top-left
            {
                int lastIndexWidth = width - 1;
                float3 normal = float3.zero;
                int numTriangleVertex = 1;

                if (KWGrid.IsCellOnCorner(x, y, int2(width)))
                {
                ...
                else if (KWGrid.IsCellOnEdge(y, width))
                {
                    numTriangleVertex = 3;
                    if (y == 0)
                ...
                else if (KWGrid.IsCellOnEdge(x, width))
                {
                    numTriangleVertex = 3;
                    if (x == 0)
        ...
        {
            [ReadOnly] public int MapSizeX;
            [ReadOnly] public NoiseData Settings;
            ...
            public void Execute(int index)
            {
                float2 coordInGrid = KWGrid.GetXY2(index, MapSizeX);
                float2 coordInWorld = coordInGrid - (MapSizeX / 2f);
                ...
                {
                    float2 sampleXY = (coordInWorld + Octaves[i]) / Settings.Scale * frequency;
